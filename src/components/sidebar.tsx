...
export function Sidebar({ profile, myQueueCount }: { profile: Profile; myQueueCount: number }) {
  ...
  const items =
    profile.role === 'admin'
      ? [
          ...navItems.slice(0, 5),
          { href: '/templates', label: 'Templates', icon: CodeIcon },
          ...navItems.slice(5, 6),
          { href: '/aor-settings', label: 'AOR Settings', icon: UsersIcon },
          ...navItems.slice(6),
        ]
      : navItems
          ...
          return (
            <Link
              ...
              className={`group flex items-center justify-between rounded-xl border px-3 py-2.5 text-sm font-medium transition-colors ${
                isActive
                  ? 'border-brand-700 bg-nex-navyLight/70 text-white shadow-[inset_4px_0_0_0_#C8102E]'
                  : 'border-transparent text-brand-400 hover:border-brand-700 hover:bg-brand-800/50 hover:text-brand-200'
              }`}
            >
              <span className="flex items-center gap-3">
                <item.icon className="h-5 w-5 shrink-0" />
...
function CodeIcon({ className }: { className?: string }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M7.5 8.25L3.75 12l3.75 3.75M16.5 8.25L20.25 12l-3.75 3.75M13.5 4.5l-3 15" />
    </svg>
  )
}
...
function CodeEditorNavIcon({ className }: { className?: string }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
